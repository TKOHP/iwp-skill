...
| [ADR-014](#adr-014本地回调-loopback-双栈绑定--交付前就绪探测2026-09-23) | 本地回调 loopback 双栈绑定 + 交付前就绪探测 | ✅ |
...
## ADR-014：本地回调 loopback 双栈绑定 + 交付前就绪探测（2026-09-23）

**决策**：本地回调 server 从单栈 `127.0.0.1` 改为 **loopback 双栈**——IPv4（`127.0.0.1`）与 IPv6（`[::1]`，`_HTTPServerV6` 显式 `IPV6_V6ONLY=1`）各起一个 `HTTPServer`，共享同一 `CallbackResult`，任一栈收到回调即完成；IPv6 绑定失败（系统禁用 IPv6）降级为仅 IPv4。`ensure_authorized` 在 Popen 之后、写入 `.oauth_next_step.json` 之前新增 `_wait_callback_ready`：IPv4 探测硬性（10s 未监听即报「启动失败」），IPv6 软性（失败仅记录），结果以 `callback_ready: {ipv4, ipv6}` 随 `auth start` 输出。

**理由**：
- 真机首装实测故障：redirect_uri 用 `localhost`，浏览器解析到 `::1` 而旧 server 只绑 `127.0.0.1`，浏览器最后一跳「localhost 拒绝连接」——双栈绑定从根上消除「连接地址 ≠ 监听地址」的不对称
- Popen 到 server 实际 bind 之间存在秒级窗口，浏览器先于监听到达同样被拒——交付 URL 前探测覆盖启动竞态这一双栈不解决的次生根因
- 绑 `[::1]` 而非 `::`：保持 loopback-only 安全边界（PKCE 是唯一代码截获防线的前提是最小暴露面）

**替代方案**：
- redirect_uri 改用 `127.0.0.1` 字面量（RFC 8252 推荐）→ 需同步改服务端白名单 / 客户端 `.env` / `.env.example` / 文档四处，任一漏改即授权失败，用新的配置漂移风险换已修复的解析风险，否决（未来如服务端配合可再评估）
- 单 server 绑 `::`（V6ONLY=0 双栈一体）→ 监听全部接口，破坏 loopback-only，否决
- 仅文档提示「若拒绝连接改用手动注入」→ 故障仍必现，恢复路径只是兜底而非修复，否决

**约束**：
- 恢复路径命令必须写等号形式 `auth finish --code=<code>`（code 常以 `-` 开头，空格形式被 argparse 当参数，见 auth-flow.md）